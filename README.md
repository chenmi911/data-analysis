...
该 repo 用来记录我的数据分析学习与作品集项目，目前已整理两个端到端项目：

- **Cookie Cats 手游 A/B 测试（最新）**：围绕真实业务决策——"是否把首个等待门槛从第 30 关后移到第 40 关"，用 90,189 名玩家的随机分流数据做因果判断。重点不只报做法，而是讲清**为什么这样设计实验、为什么选这些指标、结论如何支撑推广与否的决策**，并识别"只看到达门槛玩家"的选择偏差陷阱。
- **Superstore 零售经营分析**：记录从原始订单数据到清洗表、SQL 经营分析、图表和结论的完整常规业务分析工作流，覆盖指标口径、分组对比、折扣影响、客户分层。

每个项目都保留：数据来源与校验说明、分析脚本（SQL / Python）、过程文档、结论与边界。AB 测试侧重**实验设计与统计推断**，Superstore 侧重**指标口径与业务解释**。
...
* 训练从"数据现象"到"业务解释"再到"行动建议"的分析思维；AB 测试项目额外训练**从业务问题到实验设计、再到统计检验与稳健性诊断**的闭环。
